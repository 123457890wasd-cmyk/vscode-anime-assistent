...
| `AIRI_CARD=<模式>` | 角色底板：`off` / `tight` / `square`（默认）/ `frame` / `all`，见下文「块状白」 |
...
**这块"白"有两个来源，只有一个是真的坏了 —— 先分清是哪一个。**

| 来源 | 长什么样 | 位置 |
|---|---|---|
| 模型自带美术（**正常**） | 白色蕾丝女仆头饰、白色书桌板（上面有圣代、笔、回形针） | 角色身上 / 桌上 |
| 透明窗口的合成 bug（**已修**） | 与文字同宽的**规则矩形**、圆角，填充 `rgb(241,228,233)` | 正好盖住对话框、以及 `Airi / online` 两行 |

怎么分：用真 Chrome 单独渲染模型（`live2d_probe/pet_ui_shot.png`）—— 头饰和书桌板会照旧出现，
说明那是美术；只有"文字背后那块方底"是**只有桌宠窗口才画得出来**的。它为什么会出现，
见 `live2d_probe/README.md` 第四轮：`backdrop-filter` 在透明窗口上没有"背后"可模糊、
`filter` 会把元素提升成独立合成层、次像素抗锯齿要求不透明的底 —— 三条都已拆掉。

模型那一半**不可能靠改代码消掉**（它就在模型的贴图里）。所以要"让它别那么扎眼"，
就给它垫一张白卡 —— 白底上的白等于不存在：

| `AIRI_CARD=` | 效果 | 不透明像素占窗 |
|---|---|---|
| `off` | 不画卡（回归裸角色） | 17.50% |
| `tight` | 紧贴角色的圆角白卡（白面最少） | 27.68% |
| **`square`（默认）** | 同上但取正方形 —— 贴合"以最小的白色正方形框住" | 29.31% |
| `frame` | 只描一圈白框，卡内照旧透出桌面（白面积 0） | 17.88% |
| `all` | 连名牌一起包进同一张卡 | — |

卡的大小不是写死的：它跟着**实测的角色外接框**走（`137×124`，而不是 `.character-box`
那个 160×210），并且前 6 次采样取并集后锁定 —— 不锁的话模型一呼吸卡就抖。
五联对比图：`live2d_probe/ui_cards.png`。

> 提示：`AIRI_CARD=frame` 是"一点都不想要白面"时的答案；`all` 则是"连名牌那块也一起
> 兜住"的保险（代价是 `online` 那个灰字在白底上偏淡）。
